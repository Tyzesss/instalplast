  ...
  return (
    <>
      <button
        ...
        className={cn(
          "z-40 bg-navy/40 backdrop-blur-[2px] transition-opacity duration-300 lg:hidden",
          open ? "fixed inset-0 opacity-100" : "hidden",
        )}
      />
      ...
      >
        ...
        />
        <motion.div
          className="relative mx-auto flex h-18 max-w-[1360px] items-center justify-between gap-3 px-5 py-4 lg:px-8"
        ...
        >
          {alwaysSolid ? (
            <Link to="/" aria-label={SITE_NAME}>
              <BrandMark onDark wordmarkClassName="h-7 sm:h-8" />
            </Link>
          ) : (
